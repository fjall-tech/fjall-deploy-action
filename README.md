...
jobs:
  deploy:
    ...
    steps:
      ...
      - uses: fjall-tech/fjall-deploy-action@v7
        with:
          target: my-app
        env:
...
| `cli-version`       | no       | `17`     | —                  | Pin the published `fjall` CLI version. Defaults to this action's compatible major (floats across `17`.x, never crossing into the next major); `auto` derives the major from the app's pinned `@fjall/components-infrastructure`; `latest` always installs the newest published release |
...
- **The action ref** — `fjall-tech/fjall-deploy-action@v7` is a moving major tag, repointed at the latest 7.x release on each republish; pin an exact tag (`@v7.0.0`) for maximum determinism.
