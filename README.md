...
| `cli-version`       | no       | `5`      | —                  | Pin the published `fjall` CLI version. Defaults to this action's compatible major (floats across `5`.x, never crossing into the next major); `auto` derives the major from the app's pinned `@fjall/components-infrastructure`; `latest` always installs the newest published release |
