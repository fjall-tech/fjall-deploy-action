...
jobs:
  deploy:
    ...
    steps:
      ...
      - uses: fjall-tech/fjall-deploy-action@v24
        with:
          target: my-app
        env:
...
| `cli-version`       | no       | `34`     | —                  | Pin the published `fjall` CLI version. Defaults to this action's compatible major (floats across `34`.x, never crossing into the next major); `auto` derives the major from the app's pinned `@fjall/components-infrastructure`; `latest` always installs the newest published release |
...
- **The action ref** — `fjall-tech/fjall-deploy-action@v24` is a moving major tag, repointed at the latest 24.x release on each republish; pin an exact tag (`@v24.0.0`) for maximum determinism.
