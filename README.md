...
jobs:
  deploy:
    ...
    steps:
      ...
      - uses: fjall-tech/fjall-deploy-action@v23
        with:
          target: my-app
        env:
...
| `cli-version`       | no       | `33`     | —                  | Pin the published `fjall` CLI version. Defaults to this action's compatible major (floats across `33`.x, never crossing into the next major); `auto` derives the major from the app's pinned `@fjall/components-infrastructure`; `latest` always installs the newest published release |
...
- **The action ref** — `fjall-tech/fjall-deploy-action@v23` is a moving major tag, repointed at the latest 23.x release on each republish; pin an exact tag (`@v23.0.0`) for maximum determinism.
