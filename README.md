...
| `cli-version`       | no       | `6`      | —                  | Pin the published `fjall` CLI version. Defaults to this action's compatible major (floats across `6`.x, never crossing into the next major); `auto` derives the major from the app's pinned `@fjall/components-infrastructure`; `latest` always installs the newest published release |
