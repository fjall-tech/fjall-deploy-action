...
jobs:
  deploy:
    ...
    steps:
      ...
      - uses: fjall-tech/fjall-deploy-action@v25
        with:
          target: my-app
        env:
...
| `cli-version`       | no       | `35`     | —                  | Pin the published `fjall` CLI version. Defaults to this action's compatible major (floats across `35`.x, never crossing into the next major); `auto` derives the major from the app's pinned `@fjall/components-infrastructure`; `latest` always installs the newest published release |
...
- **The action ref** — `fjall-tech/fjall-deploy-action@v25` is a moving major tag, repointed at the latest 25.x release on each republish; pin an exact tag (`@v25.0.0`) for maximum determinism.
